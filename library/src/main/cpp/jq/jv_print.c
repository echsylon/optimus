...
static void put_indent(int n, int flags, FILE *fout, jv *strout, int T) {
    if (flags & JV_PRINT_TAB) {
        ...
        n *= ((flags & (JV_PRINT_SPACE0 | JV_PRINT_SPACE1 | JV_PRINT_SPACE2)) >>
              8);
        while (n--)
            put_char(' ', fout, strout, T);
    }
...
static void jv_dump_term(struct dtoa_context *C, jv x, int flags, int indent,
    ...
    if (flags & JV_PRINT_COLOR) {
        for (unsigned i = 0;
             i < sizeof(color_kinds) / sizeof(color_kinds[0]); i++) {
        ...
        switch (jv_get_kind(x)) {
            ...
            case JV_KIND_OBJECT: {
                ...
                while (1) {
                    ...
                    first = 0;
                    if (color)
                        put_str(FIELD_COLOR, F, S, flags & JV_PRINT_ISATTY);
