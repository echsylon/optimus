...
static jv type_error2(jv bad1, jv bad2, const char *msg) {
    ...
    jv err = jv_invalid_with_msg(jv_string_fmt("%s (%s) and %s (%s) %s",
                                               ...
                                               jv_dump_string_trunc(
                                                       jv_copy(bad1), errbuf1,
                                                       sizeof(errbuf1)),
                                               jv_kind_name(jv_get_kind(bad2)),
                                               jv_dump_string_trunc(
                                                       jv_copy(bad2), errbuf2,
                                                       sizeof(errbuf2)),
                                               msg));
    jv_free(bad1);
    jv_free(bad2);
...
static jv f_startswith(jq_state *jq, jv a, jv b) {
    if (jv_get_kind(a) != JV_KIND_STRING || jv_get_kind(b) != JV_KIND_STRING)
        return jv_invalid_with_msg(
                jv_string("startswith() requires string inputs"));
    int alen = jv_string_length_bytes(jv_copy(a));
    int blen = jv_string_length_bytes(jv_copy(b));
    jv ret;

    if (blen <= alen &&
        memcmp(jv_string_value(a), jv_string_value(b), blen) == 0)
        ret = jv_true();
    else
        ret = jv_false();
    jv_free(a);
...
static jv f_endswith(jq_state *jq, jv a, jv b) {
    if (jv_get_kind(a) != JV_KIND_STRING || jv_get_kind(b) != JV_KIND_STRING)
        return jv_invalid_with_msg(
                jv_string("endswith() requires string inputs"));
...
static jv f_ltrimstr(jq_state *jq, jv input, jv left) {
    if (jv_get_kind(f_startswith(jq, jv_copy(input), jv_copy(left))) !=
        JV_KIND_TRUE) {
        jv_free(left);
        return input;
    }
...
static jv f_rtrimstr(jq_state *jq, jv input, jv right) {
    if (jv_get_kind(f_endswith(jq, jv_copy(input), jv_copy(right))) ==
        JV_KIND_TRUE) {
...
static jv f_mod(jq_state *jq, jv input, jv a, jv b) {
    ...
    if (jv_get_kind(a) == JV_KIND_NUMBER && jv_get_kind(b) == JV_KIND_NUMBER) {
        ...
        return jv_number(
                (intmax_t) jv_number_value(a) % (intmax_t) jv_number_value(b));
    } else {
...
static jv f_format(jq_state *jq, jv input, jv fmt) {
    ...
    if (!strcmp(fmt_s, "json")) {
        ...
        jv_array_foreach(input, i, x) {
                ...
                switch (jv_get_kind(x)) {
                    ...
                    case JV_KIND_STRING: {
                        ...
                        line = jv_string_concat(line,
                                                escape_string(x, escapings));
                        line = jv_string_append_str(line, quotes);
                        break;
                    }
                    default:
        ...
        jv_free(fmt);
        return escape_string(f_tostring(jq, input),
                             "&&amp;\0<&lt;\0>&gt;\0'&apos;\0\"&quot;\0");
        ...
        input = f_tostring(jq, input);
        jv line = jv_string("");
        const unsigned char *data = (const unsigned char *) jv_string_value(
                input);
        ...
        int len = jv_string_length_bytes(jv_copy(input));
        size_t decoded_len =
                (3 * len) / 4; // 3 usable bytes for every 4 bytes of input
...
static jv f_keys(jq_state *jq, jv input) {
    if (jv_get_kind(input) == JV_KIND_OBJECT ||
        jv_get_kind(input) == JV_KIND_ARRAY) {
...
static jv f_env(jq_state *jq, jv input) {
    ...
    for (const char **e = environ; *e != NULL; e++) {
        ...
        if (val == NULL)
            ...
            env = jv_object_set(env, jv_string_sized(var, val - var),
                                jv_string(val + 1));
    }
    return env;
}
...
static jv f_string_split(jq_state *jq, jv a, jv b) {
    if (jv_get_kind(a) != JV_KIND_STRING || jv_get_kind(b) != JV_KIND_STRING) {
        ...
        jv_free(b);
        return jv_invalid_with_msg(
                jv_string("split input and separator must be strings"));
    }
    return jv_string_split(a, b);
}
...
static jv f_setpath(jq_state *jq, jv a, jv b, jv c) {
...
static jv f_modulemeta(jq_state *jq, jv a) {
    if (jv_get_kind(a) != JV_KIND_STRING) {
        jv_free(a);
        return jv_invalid_with_msg(
                jv_string("modulemeta input module name must be a string"));
    }
...
static jv f_strptime(jq_state *jq, jv a, jv b) {
    ...
    jv_free(b);
    return jv_invalid_with_msg(
            jv_string("strptime/1 not implemented on this platform"));
}
...
static jv f_mktime(jq_state *jq, jv a) {
    ...
    if (jv_array_length(jv_copy(a)) < 6)
        return jv_invalid_with_msg(
                jv_string("mktime requires parsed datetime inputs"));
    struct tm tm;
    if (!jv2tm(a, &tm))
        return jv_invalid_with_msg(
                jv_string("mktime requires parsed datetime inputs"));
    ...
    if (t == (time_t) -2)
        return jv_invalid_with_msg(
                jv_string("mktime not supported on this platform"));
    return jv_number(t);
}
...
static jv f_gmtime(jq_state *jq, jv a) {
    jv_free(a);
    return jv_invalid_with_msg(
            jv_string("gmtime not implemented on this platform"));
}
...
static jv f_strftime(jq_state *jq, jv a) {
    jv_free(a);
    return jv_invalid_with_msg(
            jv_string("strftime/1 not implemented on this platform"));
}
...
static block bind_bytecoded_builtins(block b) {
    block builtins = gen_noop();
    {
        ...
        for (unsigned i = 0;
             i < sizeof(builtin_defs) / sizeof(builtin_defs[0]); i++) {
            builtins = BLOCK(builtins,
                             gen_function(builtin_defs[i].name, gen_noop(),
                                          builtin_defs[i].code));
        }
    }
    {
        ...
        for (unsigned i = 0;
             i < sizeof(builtin_def_1arg) / sizeof(builtin_def_1arg[0]); i++) {
...
static block gen_builtin_list(block builtins) {
    jv list = jv_array_append(block_list_funcs(builtins, 1),
                              jv_string("builtins"));
    return BLOCK(builtins,
...
int builtins_bind(jq_state *jq, block *bb) {
    ...
    builtins = gen_cbinding(function_list,
                            sizeof(function_list) / sizeof(function_list[0]),
                            builtins);
