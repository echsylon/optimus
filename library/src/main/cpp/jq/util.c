...
jv expand_path(jv path) {
    ...
    jv ret = path;
    if (jv_string_length_bytes(jv_copy(path)) > 1 && pstr[0] == '~' &&
        pstr[1] == '/') {
        jv home = get_home();
        if (jv_is_valid(home)) {
            ...
            ret = jv_invalid_with_msg(
                    jv_string_fmt("Could not expand %s. (%s)", pstr,
...
jv get_home() {
    ...
    if (!home) {
        ...
        if (pwd)
            ...
            ret = jv_invalid_with_msg(
                    jv_string("Could not find home directory."));
#else
...
// If parser == NULL -> RAW
jq_util_input_state *jq_util_input_init(jq_util_msg_cb err_cb,
                                        void *err_cb_data) {
    if (err_cb == NULL) {
        err_cb = fprinter;
        err_cb_data = stderr;
...
void jq_util_input_set_parser(jq_util_input_state *state, jv_parser *parser,
                              int slurp) {
...
void jq_util_input_add_input(jq_util_input_state *state, const char *fname) {
    state->files = jv_mem_realloc(state->files, (state->nfiles + 1) *
...
static int jq_util_input_read_more(jq_util_input_state *state) {
    if (!state->current_input || feof(state->current_input) ||
        ...
        if (state->current_input) {
            if (state->current_input == stdin) {
                clearerr(
                        stdin); // perhaps we can read again; anyways, we don't fclose(stdin)
            } else {
    ...
    if (state->current_input) {
        ...
        while (!(res = fgets(state->buf, sizeof(state->buf),
                             state->current_input)) &&
    ...
    return state->curr_file == state->nfiles &&
           (!state->current_input || feof(state->current_input) ||
            ferror(state->current_input));
}
...
jv jq_util_input_get_position(jq_state *jq) {
    ...
    if (cb != jq_util_input_next_input_cb)
        return jv_invalid_with_msg(
                jv_string("Invalid jq_util_input API usage"));
    ...
    jv v = jv_string_fmt("%s:%lu", jv_string_value(s->current_filename),
